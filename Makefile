PROJECT_NAME := TLS Package
include build/common.mk

PACK             := tls
PACKDIR          := sdk
PROJECT          := github.com/pulumi/pulumi-tls
NODE_MODULE_NAME := @pulumi/tls
...
provider:: generate_schema
	...
	cd provider && go install -ldflags "-X github.com/pulumi/pulumi-tls/provider/v2/pkg/version.Version=${VERSION}" ${PROJECT}/provider/v2/cmd/${PROVIDER}

tfgen::
	cd provider && go install -ldflags "-X github.com/pulumi/pulumi-tls/provider/v2/pkg/version.Version=${VERSION}" ${PROJECT}/provider/v2/cmd/${TFGEN}
